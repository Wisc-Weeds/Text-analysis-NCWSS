...
freq_by_rank <- freq_by_rank %>% 
  mutate(highlight = ifelse(word %in% c("palmer", "waterhemp", "dicamba", "2,4-D", "resistant",
                                            "drift"), TRUE, FALSE),
...
theme_style <- theme(text = element_text(family = font_family),
                  ...
                  axis.title.y = element_text(face = 'bold', size = 40, colour = text_colour),
                  axis.title.x = element_blank(),
                  axis.text.x = element_text(size = 35, colour= axis_colour, vjust = 14),
                  axis.text.y = element_text(size = 25, colour = text_colour),
...
#Set colour palette
cols <- c("#FE5F55", "#4281A4", "#48A9A6", "#39393A", "#D4B483", "#E4DFDA", "#C1666B")
```
...
ggplot(freq_by_rank_1, aes(x = year, y = rank, group = word)) +
  ...
  labs(title = "A NCWSS Meeting text analysis of title and abstracts from 2001 through 2019",
       subtitle = "", y= "Rank",

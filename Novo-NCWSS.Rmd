...
for (i in 1:length(rm_author_06)) {
  print(i)
...
freq_by_rank <- freq_by_rank %>% 
  mutate(highlight = ifelse(word %in% c("control", "glyphosate", "dicamba", "2,4-D", "resistance",
                                            "horseweed"), TRUE, FALSE),
...
theme_style <- theme(text = element_text(family = font_family),
                  ...
                  plot.subtitle = element_text(size = 25, colour = text_colour),
                  ...
                  axis.title.y = element_text(face = 'bold', size = 35, colour = text_colour),
                  axis.title.x = element_blank(),
                  axis.text.x = element_text(size = 35, colour= axis_colour, vjust = 14),
                  axis.text.y = element_text(size = 35, colour = text_colour),
...
#Set colour palette
cols <- c("#FE5F55", "#4281A4", "#48A9A6", "#D4B483", "#E4DFDA", "#39393A", "#C1666B")
```
...
freq_by_rank <- freq_by_rank %>% 
  filter(rank < 101) 
ggplot(freq_by_rank, aes(x = year, y = rank, group = word)) +
  ...
  scale_x_continuous(breaks = seq(2001, 2019, 2), limits= c(2000, 2020), 
                     expand = c(.05, .05)) +
  geom_text(data = freq_by_rank %>%  filter(year == "2001"),
            aes(label = word, x = 2000.8, color = variable_col),
            hjust = "right",
            fontface = "bold",
            size = 7) +
  geom_text(data = freq_by_rank %>%  filter(year == "2019"),
  ...
  labs(title = "NCWSS word frequency in the XXI century ",
       subtitle = "A NCWSS Meeting text analysis of title and abstracts from 2001 through 2019",
       caption = "Vizualisation: @maxwelco | Source: NCWSS")

#Export plot
ggsave("word_rank.png", width = 30, height = 45, dpi=600)
```
